...
A **cargo unit** is one file, or one folder treated as a whole, in `_INBOX/` (legacy fallback: the repo root for `DOCTRINE_PROMOTION*`, `*PATCH*`, `LESSONS*`). A correction package arrives as a folder tree and is ONE unit at the package level; each correction inside it is one ledger ROW. Recognition is by discovery, never by asking Tony what a file is, and it is **content-first**. Read the unit and decide from its **status** (APPROVED, FLAGGED, DRAFT, historical), **contents**, **intent** (what it asks to become true), **target** (which doc, repo, or folder it aims at), **provenance** (who produced it, from which run or retro), **authority** (who approved it, and whether that approval governs doctrine), and **semantic form** (brief, entry, whole document, image, record), plus a Hub-counterpart match by canonical name first and content similarity second. The **filename is supporting evidence only**: a file is not a lesson because "LESSONS" is in its name, and a file named otherwise is not excluded from a class its contents prove. When filename and contents disagree, contents win and the disagreement is stated aloud.
...
| Pre-existing / baseline drift (a derived, index, or infra value already stale at BASE_SHA that the intake does not authorize repairing) | `DRIFT-<run>-<NN>` | Minted at discovery or Phase 2; recorded in map §G with BASE evidence; parked for follow-up, never on the touch list (D7) |
...
| 2 | **DOCTRINE JOURNAL / LESSON** | Contents are lesson or promotion entries approved for doctrine (e.g. FLAGGED by a retrospective), each with a target doc + placement + content, or an intent-level rule change. Names like `DOCTRINE_PROMOTION*`, `*PATCH*`, `LESSONS*`, `*JOURNAL*` are supporting evidence only — a file so named whose contents are a run record is class 7 | One row per entry (`IN-…`). Edit-level entries: verify placement against live structure, land content exactly, translate mechanics (D15). Intent-level entries: draft proposed wording in the map like a correction. Propagation search for any entry that changes a rule. |
...
| 11 | **BLOCKED / NEEDS OPERATOR DECISION** | Unclassifiable unit; ambiguous counterpart match; conflicting corrections; a cited doc the Hub lacks with no doc riding in; an invariant that cannot hold without breaking a preservation constraint; ambiguous tier ownership | A BLOCKED row with the exact question, the evidence, and what it blocks. Presented at the nearest gate. Never guessed, never silently resolved. Tony may consult Jarvis for architecture / preservation / semantic adjudication (D17) — Claudy does not. **Not this class:** a MECHANICAL CLARIFICATION (a non-doctrinal detail such as which real anchor a moved placement maps to): Claudy resolves it from evidence first and, only if disk cannot settle it, asks the smallest question as a §K CLARIFICATION row — the item keeps its real class and the run is not BLOCKED (D17). |
