...
Do the family CLAUDE.md §2 activation sequence exactly: seat resolution → CLAUDE.md → this file → `RECOVERY.md` + session file → environment discovery (`pwd`, `ls`, `git remote -v`, branch, HEAD, status, `git fetch`, distance from main; proposed BASE_SHA with its evidence; RESUME detection; `_INBOX/` cargo scan; MANIFEST read + live-doc count, with any mismatch at BASE recorded as baseline drift; lint baseline; MCP presence). Read `_shared/references/ANTI_PATTERNS.md` and name the one or two failure modes most relevant to the job's shape.
...
1. For each cargo unit in `_INBOX/` (a folder is one unit; a correction package is one unit with one ledger row per correction), read its contents and classify aloud into exactly one of the eleven classes in `_shared/references/INTAKE_TAXONOMY.md`. Classification is content-first (D16): state the evidence from status, contents, intent, target, provenance, authority, and semantic form, plus the Hub counterpart match (canonical name first, content similarity second). The filename is supporting evidence only and is never the sole reason for a class.
...
6. Present the Phase 1 plan: ledger · proposed BASE_SHA with evidence · splits parked · gaps · baseline drift parked (`DRIFT-…`) · CONFLICTs (BLOCKED, exact) and mechanical CLARIFICATIONs (only those evidence could not settle, each the smallest question) · provisional route with evidence · what happens next.
...
1. Open `_shared/templates/UPDATE_MAP.md`; create `_AUDIT/SYNC_<YYYY-MM-DD>_<slug>/UPDATE_MAP.md` (the durable folder is created now, on the working tree, as a run-state / planning write per D2; it is committed with the run). Fill §0 (BASE_SHA — the commit every placement below is verified against, D18; run branch and merge target; lint baseline; live-doc count vs MANIFEST).
...
4. **Draft the wording (§C):** for every touch-list row carry the verified placement and the **proposed wording** — full text for new or replaced sentences, "delete L a–b" for removals, a precise intent line only for mechanical edits such as a rename. Edit-level cargo lands content exactly as written with mechanics translated (D15: canonical names, single-line header, kit-internal references resolved to real sections and versions — flag every translation). Exact intake wording is preserved verbatim unless Tony approves otherwise. Every word you add — heading, wrapper text, transition, label, explanation, normalization — is written out in the row and marked **ENGINEER-PROPOSED**, so Tony can approve or strike it at Gate 2 (D15). Prefer the minimal form on high-← docs and say so (D14). This wording is what Tony approves; Phase 4 applies it EXACTLY.
5. **NO-CHANGE proposals** carry path:line evidence that the intent is already true; they are valid only when Tony approves the row (D17). **CONFLICT** — a doctrine / authority conflict (evidence contradicts an approved item; two items collide on one passage; an invariant breaks a preservation constraint; an approved request cannot coexist with governing doctrine) → §K type CONFLICT, BLOCKED, with the exact question. You never resolve it. A **mechanical clarification** (a non-doctrinal implementation detail) is not a conflict: resolve it from evidence first; only if disk cannot settle it, record it in §K type CLARIFICATION as the smallest necessary question (D17).
6. Fill §D new files (owning tier by the doc's subject; ambiguous → §K per Ruling 10), §E stale material, §F dependents checked (examples, checklists, templates, quick references, role instructions — each with a CONSISTENT quote or a move to §C), §G infra effects (rows FROM headers; count, ← map, appendix, CHANGELOG rows — updated only where this run changes the underlying value; Date-only on MANIFEST/CHANGELOG, no archive) and §G baseline drift (every derived/index/infra value already stale at BASE_SHA that the intake does not authorize repairing: `DRIFT-…` ID, BASE evidence, parked — not on the touch list, D7), §H paths/links/assets (`<tier>/_assets/`, case-exact, source vs rendered, replacement archived by date), §I skill references (awareness only), §J parked, §L preservation consolidated, §M expected validation (baseline, searches to re-run verbatim, term lists, derived-field assertions, archive count), §N commit plan.
...
**⛔ Stop Gate 2:** "Awaiting your APPROVED on the UPDATE MAP" (including BASE_SHA, every ENGINEER-PROPOSED wording span, NO-CHANGE rows, parked drift, the route, and — TINY — the waiver). Record the approval in §O. After Gate 2 the map is frozen; any change is an AMENDED-v<n> that re-enters Gate 2.
...
**⛔ Stop Gate 3 (Tony approves the spec; it freezes).** You wait for the Operator's word that Gate 3 passed before starting Phase 4.
...
1. **Branch from the approved BASE_SHA (D18), never blindly from main:** confirm the tree is clean and `git cat-file -e <BASE_SHA>^{commit}` succeeds. If you are already on the approved run branch (map §0) and `git merge-base --is-ancestor <BASE_SHA> HEAD` succeeds with only this run's own commits since BASE_SHA (`git log <BASE_SHA>..HEAD`), stay — no new branch. Otherwise create it from the base: `git switch -c <run-branch> <BASE_SHA>` (default name `docs/sync-<slug>-<YYYY-MM-DD>`). HEAD not descended from BASE_SHA, or unexplained commits after it → STOP and report (D15a, D22). Type commands by hand (AP-10).
...
4. **After the last doc:** recompute from disk the MANIFEST derived fields this run changed, exactly as map §G lists them (live-doc count when a canonical doc was added or removed, ← map for changed Pairs-with, appendix scope note when its scope changed); leave every §G `DRIFT-…` value exactly as it was at BASE_SHA (D7); bump MANIFEST and CHANGELOG header Dates (no archive, Ruling 7); commit as part of the last doc's commit or as `docs(MANIFEST): recompute derived fields [<run-id>]`. Re-run the §B searches at HEAD and confirm every CHANGE hit is gone and every CONSISTENT hit still holds.
...
**After APPROVED, and only then:** `git push -u origin <run-branch>` — the one authorized push (D18; `_shared/references/SHA_AND_PUSH_CONTRACT.md`). Report `git rev-parse origin/<branch>` aloud.
...
1. Report the compare URL: `https://github.com/ahmedmusawir/app-factory-docs-v1/compare/<merge-target>...<branch>` — `<merge-target>` from map §0 (`main` for a production sync) (his box has no `gh` CLI; one click).
...
1. **Verify the merge landed:** `git checkout <merge-target> && git pull` (PULL, never push, on main or any merge target); `git log --oneline -<n>` shows the run's commits; find the PR number deterministically (`git ls-remote origin 'refs/pull/*/head'` matched to the pushed tip).
...
7. Update `RECOVERY.md` (last action, pending, next step, parked XB and DRIFT items by ID) and close the session file entry.
...
> **Agent:** Reading CLAUDE.md… done; no seat named, so I am Claudy. Reading sync-engineer/SKILL.md… done. RECOVERY.md says the last run closed clean. Discovery: Hub clone, on main, tree clean, main is current with origin, launch line names no other base — proposed BASE_SHA is the current HEAD, `4c3b2a1`. Lint baseline: nine pre-existing findings, all in STARTER_KIT_HANDBOOK, DATABASE_MANUAL, UI_UX_BUILDING_MANUAL — recorded. MANIFEST says 29 live docs; disk has 31 at BASE — nothing in the intake adds or removes a doc or authorizes that repair, so it is baseline drift DRIFT-2026-09-21-01, parked, not on the touch list. Cargo scan: `_INBOX/` holds one folder, `CORRECTION_PACKAGE_2026-09/`, and one file, `LESSONS_ADK_HARNESS.md`. Reading contents: the folder has a master index with status APPROVED and three briefs, each carrying problem, evidence, intent, invariants, and preservation — an APPROVED CORRECTION PACKAGE, CP-01 through CP-03, all fields present. The file's contents are two retrospective-FLAGGED entries, each naming a target doc, a placement, and exact wording — a DOCTRINE JOURNAL by its contents, not just its name; entries become IN-2026-09-21-01 and -02. CP-02 renames a gate across the Factory; that alone fails the fast path. Provisional route: LARGE. One question I cannot answer from disk: IN-…-02 cites `TRM_PLAYBOOK.md`, which the Hub does not have — GAP-…-01; does a doc ride in, or does it land flagged? Awaiting your APPROVED on scope and classification.
> **Operator:** Lands flagged. Approved.
> **Agent:** Phase 2. For CP-02 the search terms are the old gate name, the new gate name, and "pre-deployment gate"; scope is the five tiers plus MANIFEST and CHANGELOG. Fourteen hits: nine CHANGE across four docs, three CONSISTENT, two HISTORY. Placements verified — the FFM_PLAYBOOK checklist calls it "Stage 7 Completion Checklist," not "phase-close checklist" as the brief says; anchoring there and flagging it. Proposed wording per row… IN-…-01's sentence lands verbatim; the one-line lead-in I added before it is marked ENGINEER-PROPOSED — approve or strike it. CP-03 is already true at SOFTWARE_FACTORY_PLAYBOOK §2.5 item 5 — proposing NO-CHANGE with the quote; your approval makes it so. Route with the finished map: five touch-list docs, a Factory-wide rename — LARGE, no downgrade proposed. Awaiting your APPROVED on the UPDATE MAP.
...
> *(later)* **Agent:** Gate 3 passed. Branch created from the approved BASE_SHA `4c3b2a1`. Lightest first: FEAT_PLAYBOOK — archived as `_ARCHIVE/FEAT_PLAYBOOK_v1_0.md`, fidelity check empty, edited per §C rows 1–2, header 1.0 → 1.1, lints clean, committed. … Last content commit done; CONTENT_SHA recorded. Handoff written and committed. Nothing pushed. Gate 4 evidence: five docs, five archives, MANIFEST rows for the five equal their headers, stated count untouched at 29 (DRIFT-…-01, parked), CHANGELOG five rows, lints zero new, baseline nine unchanged. Awaiting your APPROVED to publish the branch.
> **Operator:** Approved.
...
2. **Drafting wording in Phase 4, or hiding it in the map.** All wording is proposed in the map and approved at Gate 2; Phase 4 applies it exactly. Any word you add to intake-supplied wording is marked ENGINEER-PROPOSED in its §C row (D15).
...
9. **"While I'm in here" fixes** — baseline lint reds, typos in untouched docs, an index count that was already stale at BASE, tempting refactors → CONCERNS, `DRIFT-…` rows, or follow-up findings. Never the diff.
...
13. **Branching from main by habit.** The approved BASE_SHA is the execution base; main is only the base when the map says so (D18).
14. **Escalating mechanics as conflict.** A placement anchor or a row format is a mechanical clarification — evidence first, then the smallest question; CONFLICT is for incompatible governing requirements (D17).
15. **Classifying by filename.** "LESSONS" in a name proves nothing; contents, status, intent, and authority decide the class (D16).
...
| Version | Date | Change |
|---------|------|--------|
| 0.7-DRAFT | 2026-09-14 | TEST A hardening (bounded). P0/P1: proposed BASE_SHA from discovery; content-first classification (filename supporting only); baseline drift parked as `DRIFT-…`; CONFLICT vs mechanical CLARIFICATION presented separately. P2: map is a run-state / planning write; BASE_SHA recorded in §0 before Gate 2 (no longer "pending until branch creation"); ENGINEER-PROPOSED wording marked in §C; §K typed CONFLICT / CLARIFICATION; §G updates only run-caused derived fields, drift parked. P4: branch from the approved BASE_SHA or reuse the run branch at BASE / a lawful descendant — the `git checkout main && git pull` branch step removed; drift values left as at BASE. P5/P8/P9: `<run-branch>` and `<merge-target>` replace hard-coded names. Worked example and anti-patterns 2, 9, 13–15 updated. IMPLEMENTED — AWAITING INDEPENDENT VALIDATION. |
