...
Every conflict that surfaced at the Cyber Pharma v1 Phase 1 Discovery gate traced to the same root cause: **the Architect authored the FFM without ground-truth access to the actual repo.** The FFM was written from operator descriptions, the starter kit handbook, and project-knowledge docs — and some of those sources were themselves wrong.

Conflicts that a recon pass would have caught up front (all from Cyber Pharma v1 Phase 1):

| Conflict | Root cause | Recon question that catches it |
|---|---|---|
| Next.js 15 vs 16 | Architect used global default, never checked `package.json` | Q1.1–1.3 (stack versions) |
| authService wrapper vs kit's complete auth | Architect trusted DATA_CONTRACT over the kit's actual implementation | Q3.1–3.4 (auth pattern) |
| `app-role.ts` ghost file | Handbook claimed a file that doesn't exist; Architect inherited the lie | Q2.1–2.3 (kit structure vs handbook claims) |
| `src/services/` exists unexpectedly | No one checked what the kit already ships | Q2.4 (existing directories) |
| Demo "posts" scaffolding cascade (11 files) | No one checked what tutorial filler the kit ships | Q8.1–8.3 (demo/tutorial scaffolding) — NEW in v0.2 |
| `useAuthStore` missing derived flags (`isAdmin` etc.) | Handbook claimed flags that don't exist on disk | Q3.6 (store shape) — NEW in v0.2 |
| `useAuthStore.user` typed `any` | Latent forbidden-zone violation in the kit since init | Q3.7 (type-safety smells) — NEW in v0.2 |
| FFM templates break `tsc --noEmit` | FFM's `.ts` template stubs caught in compile scope | Q9.1 (tsconfig scope) — NEW in v0.2 |
| Skills resolve from wrong CWD | Skills load relative to launch dir, not repo root | Q6.5 (launch CWD) — already in v0.1, elevated |

**The pattern:** none of these were deep design failures. They were all "the Architect couldn't see the ground truth." The fix is cheap: have the Engineer (Claudy, who IS in the repo) answer a questionnaire before the Architect writes a line.

**The meta-lesson (the biggest one):** the **starter kit handbook is aspirational, not literal.** By the end of Phase 1, FOUR separate handbook claims were proven false against on-disk reality (the `app-role.ts` location, the "files already separated" split, the `useAuthStore` derived flags, and the `user` type). Treat the handbook as a description of *intent*, and verify every load-bearing claim against the filesystem during recon. **Where the handbook and the filesystem disagree, the filesystem wins — every time.**

---

## How To Use This

### The Recon Pass (Stage 0 of FFM authoring)

```
1. Architect (Claude in chat) hands the operator this questionnaire,
   tuned for the specific project/phase being authored.

2. Operator (you) runs Claudy in the repo:
   "Answer the RECON QUESTIONNAIRE from ground truth. Read-only —
    no file changes, no git operations. Report findings verbatim."

3. Claudy answers from the filesystem — actual versions, actual file
   locations, actual kit contents, actual grep results.

4. Architect authors the FFM from VERIFIED FACTS.
   No more "the handbook claims..." — now it's "confirmed on disk..."
```

### Discipline

- **Read-only.** Claudy answers by reading and grepping. No file changes, no `cd && git` that could trigger hooks. Pure inspection.
- **Verbatim findings.** Claudy reports what it actually found, including when reality contradicts the handbook. Doc-vs-reality drift is the single highest-value finding.
- **One pass.** The questionnaire is answered in one shot and handed back. The Architect then authors. Don't trickle questions one at a time.
...
|---|---|---|
| 0.1 | 2026-06-05 | Initial questionnaire, authored mid-Phase-1 from the four conflicts surfaced at Cyber Pharma v1 Phase 1 Discovery (Next 15/16, authService wrapper, app-role.ts ghost file, unexpected src/services/). Seven sections + recon report format. |
...
## Lessons Backlog (Promote To Sections As They Recur)

> When a NEW conflict surfaces at a Discovery gate, add a question here that would have caught it. When the same conflict recurs across two runs, promote it into a numbered section above.

Phase 1 lessons already promoted into numbered sections (v0.2): demo scaffolding cascade (§8), FFM packaging/tsconfig (§9), auth-store shape + type smells (§3.6/3.7).

Still in backlog (watch for recurrence):
- **Handbook-as-fiction frequency** — Phase 1 hit FOUR false handbook claims. If the next run also finds multiples, promote a dedicated "Handbook Audit" section that walks every handbook claim against disk systematically, not opportunistically.
- **Prior-session-log mining** — the kit's prior build logs flagged "legacy/do not touch" dirs. Q8.4 covers this, but if more lives in old session logs, consider a dedicated session-log recon question.
- _(Phase 1 retrospective will add more here at close)_

---

🛡️ **The Architect authors blind no more. Claudy verifies ground truth; the Architect writes from facts.**
